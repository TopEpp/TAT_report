var Upload=function(){
    ...
    var method= {
        setup:function(_elem,_return,_options){
            ...
            $element.dropzone(
                {
                    previewTemplate:['<div class="dz-preview dz-file-preview">',
                        ...
                        '</div>'].join(''),
                    url: domain+'services/attach/file',
                    maxFilesize: 200, // MB
                    // addRemoveLinks:true,
                    init: function () {
                        ...
                        if(options.images){
                            $.each(options.images,function(i,image){
                                $.ajax({
                                    url: domain+'services/attach/getFile',
                                    type: 'POST',
                                    dataType:'json',
                                    data: {file:image},
                       ...
                       // console.log(file,_ref3);
                        for (_k = 0, _len2 = _ref3.length; _k < _len2; _k++) {
                            ...
                            if(response.values.file){
                                removeLink.href= domain+"/"+response.values.file;
                            }
                        }
                        var file_name = response.values.file+'#$#'+file.name;
...
//                        console.log(file,_ref3);
                        for (_k = 0, _len2 = _ref3.length; _k < _len2; _k++) {
                            removeLink = _ref3[_k];
                            if(response.values.file){
                                removeLink.href="/"+response.values.file;
                            }
                        }
                    },
                    removedfile:function(file){
